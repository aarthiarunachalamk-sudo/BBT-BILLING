...
class UserProfileScreen extends StatelessWidget { const UserProfileScreen(this.state, {super.key}); final UserState state;
  @override Widget build(BuildContext context) { final u = state.user; return UserShell(state: state, title: 'Profile', child: ListView(padding: const EdgeInsets.all(16), children: [
    ...
    UserCard(onTap: () => state.go(UserPage.shelfAging), child: const ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.inventory, color: userBlue), title: Text('Stock Activity'), trailing: Icon(Icons.chevron_right))),
    UserCard(onTap: () => state.go(UserPage.quantityReview), child: const ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.history, color: userBlue), title: Text('User Activity'), trailing: Icon(Icons.chevron_right))), const SizedBox(height: 14),
