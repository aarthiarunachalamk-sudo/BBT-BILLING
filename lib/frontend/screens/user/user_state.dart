...
class UserState extends ChangeNotifier {
    ...
    await _perform(() async {
      switch (page) {
        ...
        case UserPage.stockMovement:
          stockMovements = await api.getList(
            'inventory/stock-movements',
            query: _movementDateQuery(),
          );
          break;
  ...
  void go(UserPage value, {bool load = true, bool remember = true}) {
    ...
    navIndex = switch (value) {
      ...
      UserPage.inventory ||
      UserPage.addProduct ||
      UserPage.currentStock ||
      UserPage.shelfAging ||
      UserPage.stockMovement ||
      UserPage.quantityReview ||
      UserPage.expiry => 1,
  ...
  void setReviewFilter(String value) {
    reviewFilter = value;
    notifyListeners();
  }
  ...
  List<Map<String, dynamic>> get visibleProducts => products.where((p) {
    ...
    final matches =
        q.isEmpty ||
        '${p['name']} ${p['sku']} ${p['barcode']}'.toLowerCase().contains(q);
    final status = p['stock_status']?.toString() ?? '';
    final statusMatch =
        stockFilter == 'All' ||
        status.replaceAll('_', ' ').toLowerCase() == stockFilter.toLowerCase();
  ...
  Future<bool> reviewQuantity(
    int itemId,
    int physicalQuantity, {
    String reason = 'Staff physical stock review',
  }) async => _perform(() async {
  ...
  Future<bool> moveToShelf(int itemId, int quantity) async {
    ...
    notifyListeners();
    try {
      await api.post('inventory/move-to-shelf', {
        'product_id': itemId,
        'quantity': quantity,
      });
      try {
  ...
  Future<bool> shelfProductAction(
    int productId,
    String action,
  ...
  Future<BulkShelfActionResult> bulkShelfProductAction(
    List<int> productIds,
    String action,
    Map<String, dynamic> values,
  ) async {
